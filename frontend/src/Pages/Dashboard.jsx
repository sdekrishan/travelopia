...
const Dashboard = () => {
  ...
  useEffect(() => {
    setLoading(true);
    axios
      .get(`https://travelopia-3psd.onrender.com/booking?page=1`)
      .then((res) => {
        console.log(res);
  ...
  // to get the data on page increase and decrease
  const getPagePosts = (p) => {
    setCurrentPage(p);
    setLoading(true);
    axios
      .get(`https://travelopia-3psd.onrender.com/booking?page=${p}`)
      .then((res) => {
        setLoading(false);
        setBookings(res.data.allBookings);
      })
    ...
    <>
      ...
      <Flex justifyContent={"space-between"}>
        <Button
          ...
          onClick={() => getPagePosts(currentPage)}
      ...
      <Table>
        <Thead>
          <Tr bgColor={"#BB86FC"}>
            <Th color="black" fontSize={"sm"} border="1px solid black">
              Booking Id
            </Th>
            <Th color="black" fontSize={"sm"} border="1px solid black">
              Booking Date
      ...
      <Flex
        ...
        padding=".5rem 1rem"
      >
        <Button colorScheme="orange" onClick={downloadCSV}>
          Download CSV
        </Button>
        <Flex alignItems={"center"} gap="1rem">
          <Button
            ...
            onClick={() => getPagePosts(currentPage - 1)}
          >
            Prev
          </Button>
          <Text>
            {currentPage}/{Math.ceil(totalBooking.length / 5)}
          </Text>
          <Button
            size={"sm"}
            colorScheme="orange"
            onClick={() => getPagePosts(currentPage + 1)}
            isDisabled={currentPage === Math.ceil(totalBooking.length / 5)}
          >
            Next
          </Button>
        </Flex>
        <Box>
          Total Budget - {bookings.reduce((acc, el) => el.budget + acc, 0)}
        </Box>
      </Flex>
    </>
  );
